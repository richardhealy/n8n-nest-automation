"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.WorkflowHistoryController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const workflow_history_service_1 = require("../services/workflow-history.service");
const jwt_auth_guard_1 = require("../../auth/guards/jwt-auth.guard");
let WorkflowHistoryController = class WorkflowHistoryController {
    constructor(workflowHistoryService) {
        this.workflowHistoryService = workflowHistoryService;
    }
    async getWorkflowHistory(workflowId) {
        return this.workflowHistoryService.getWorkflowHistory(workflowId);
    }
    async createSchedule(scheduleData) {
        return this.workflowHistoryService.createSchedule(scheduleData);
    }
    async updateScheduleStatus(id, isActive) {
        return this.workflowHistoryService.updateScheduleStatus(id, isActive);
    }
};
exports.WorkflowHistoryController = WorkflowHistoryController;
__decorate([
    (0, common_1.Get)(':workflowId/history'),
    (0, swagger_1.ApiOperation)({ summary: 'Get workflow execution history' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Returns workflow execution history' }),
    __param(0, (0, common_1.Param)('workflowId')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String]),
    __metadata("design:returntype", Promise)
], WorkflowHistoryController.prototype, "getWorkflowHistory", null);
__decorate([
    (0, common_1.Post)('schedule'),
    (0, swagger_1.ApiOperation)({ summary: 'Create workflow schedule' }),
    (0, swagger_1.ApiResponse)({ status: 201, description: 'Schedule created successfully' }),
    __param(0, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", Promise)
], WorkflowHistoryController.prototype, "createSchedule", null);
__decorate([
    (0, common_1.Patch)('schedule/:id/status'),
    (0, swagger_1.ApiOperation)({ summary: 'Update schedule status' }),
    (0, swagger_1.ApiResponse)({ status: 200, description: 'Schedule status updated successfully' }),
    __param(0, (0, common_1.Param)('id')),
    __param(1, (0, common_1.Body)('isActive')),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, Boolean]),
    __metadata("design:returntype", Promise)
], WorkflowHistoryController.prototype, "updateScheduleStatus", null);
exports.WorkflowHistoryController = WorkflowHistoryController = __decorate([
    (0, swagger_1.ApiTags)('workflow-history'),
    (0, common_1.Controller)('workflow-history'),
    (0, common_1.UseGuards)(jwt_auth_guard_1.JwtAuthGuard),
    __metadata("design:paramtypes", [workflow_history_service_1.WorkflowHistoryService])
], WorkflowHistoryController);
//# sourceMappingURL=workflow-history.controller.js.map